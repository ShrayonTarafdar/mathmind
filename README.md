...
QLORA Model: Shrayon/mathmind_qlora

GRPO Model: Shrayon/mathmind_grpo

ORPO Model: Shrayon/mathmind_orpo

1. QLORA (Quantized Low-Rank Adaptation) Fine-tuning
   1.1. What is QLORA?

QLORA (Quantized Low-Rank Adaptation) is a highly efficient fine-tuning technique that makes it possible to tune large language models on consumer-grade hardware.[1] It works by quantizing the main model to a lower precision (like 4-bit) to reduce memory usage, freezing its weights, and then training a small number of "adapter" matrices, a method known as Low-Rank Adaptation (LoRA).[1][2] Gradients are backpropagated through the frozen, quantized model into these LoRA adapters.[3] This approach significantly lowers the GPU memory requirements and speeds up training while preserving the performance levels of full 16-bit fine-tuning.[1][3]
...
Group Relative Policy Optimization (GRPO) is an advanced reinforcement learning technique that enhances preference optimization.[4][5] Unlike methods that compare a single "chosen" response against a single "rejected" one (like DPO), GRPO evaluates a group of generated responses for a given prompt simultaneously.[4] It then uses relative rewards within this group to guide the model, creating a more stable and robust learning signal that leads to better overall performance.[4][5]
...
Odds Ratio Preference Optimization (ORPO) is a novel and efficient technique that merges supervised fine-tuning (SFT) and preference alignment into a single, monolithic process.[6][7][8] Unlike DPO or RLHF which are separate alignment steps, ORPO modifies the standard training objective.[7] It adds a penalty term based on the odds ratio, which weakly penalizes rejected responses while strongly rewarding chosen ones.[8][9] This allows the model to learn the desired task and align with human preferences simultaneously, making it more efficient than multi-stage pipelines.[6][10]
...
beta 0.1 This is the key hyperparameter in ORPO.[8] It controls the trade-off between the standard SFT loss and the preference alignment loss.[11] A value of 0.1 gives more weight to learning the task (SFT) while still applying a gentle penalty to rejected answers, which is a recommended starting point.
